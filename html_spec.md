...
> 新生成流程以 `templates/study/v1/README.md` 的固定模板、目录与验收规则为准。无字封面用于首页课程卡片，保存在 `html/thumbs/`，不得插入课程正文；正文只放 `html/assets/阶段名/` 下的详情图。小红书输出到 `xhs/dayNN/`，不再新增按 Day 定制的渲染分支。下文旧脚本与旧推送节奏仅供历史参考。
...
- `caption.txt` 必须在末尾附当天正文短链，格式 `完整学习页：https://fitstudy.cn/go/NN/`；链接由生成脚本根据 Day 编号写入，不能手填或省略。生成正式包时同步创建 `go/NN/index.html` 跳转到实际 HTML 文件。
